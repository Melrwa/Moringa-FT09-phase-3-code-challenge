...
class Article:
    def __init__(self, id=None, title=None, content=None, author_id=None, magazine_id=None):
        ...
        """
        if id is not None:
            # Fetch an existing article from the database using its ID
            cursor.execute("""
            ...
            if result:
                ...
                raise ValueError(f"Article with id {id} not found in the database.")
        else:
            # Create a new article and validate inputs
            ...
            # Verify that the author and magazine exist in the database
            try:
